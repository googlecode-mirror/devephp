...
class Model extends Deve
{
    ...
    public function __construct($name='')
    {
        ...
        // 设置表前缀
        $this->tablePrefix = $this->tablePrefix?$this->tablePrefix:S('DB_PREFIX');
        $this->tableSuffix = $this->tableSuffix?$this->tableSuffix:S('DB_SUFFIX');
        // 字段检测
    ...
    public function flush() {
        ...
        // 记录字段类型信息
        if(S('DB_FIELDTYPE_CHECK'))   $this->fields['_type'] =  $type;
        ...
        if(!empty($this->fields)) {
            foreach ($data as $key=>$val){
                if(!in_array($key,$this->fields,true)){
                    unset($data[$key]);
                }elseif(S('DB_FIELDTYPE_CHECK') && is_scalar($val)) {
                    // 字段类型检查
     ...
     */
     public function create($data='',$type='') {
        ...
        // 表单令牌验证
        if(S('TOKEN_ON') && !$this->autoCheckToken($data)) {
