 ...
 * @version  $Id$
 +------------------------------------------------------------------------------
 */
class  ThinkTemplate extends Deve
{//类定义开始
    ...
    protected function compiler( $tmplContent)
    {
        ...
        // 添加安全代码
        $tmplContent  =  '<?php if (!defined(\'DEVE_PATH\')) exit();?>'.$tmplContent;
        if(S('TMPL_STRIP_SPACE')) {
            /* 去除html空格与换行 */
            $find     = array("~>\s+<~","~>(\s+\n|\r)~");
            $replace  = array("><",">");
    ...
    public function parse($content)
    {
        ...
        if(S('TAGLIB_LOAD')) {
            ...
            if(!empty($this->tagLib)) {
                // 对导入的TagLib进行解析
                $_taglibs = S('_taglibs_');
                foreach($this->tagLib as $tagLibName) {
                    // 内置标签库
                    if(!import('Think.TagLib.TagLib'.ucwords(strtolower($tagLibName)))) {
                        // 扩展标签库
        ...
        if(S('TAGLIB_PRE_LOAD')) {
            $tagLibs =  explode(',',S('TAGLIB_PRE_LOAD'));
            foreach ($taglibs as $tag){
                $this->parseTagLib($tag,$content);
            }
        }
        // 内置标签库 无需使用taglib标签导入就可以使用
        $tagLibs =  explode(',',S('TAGLIB_BUILD_IN'));
        foreach ($tagLibs as $tag){
    ...
    public function parseTagLib($tagLib,&$content,$hide=false)
    {
        $begin = $this->config['taglib_begin'];
        $end   = $this->config['taglib_end'];
        $tLib =  Deve::instance('TagLib'.ucwords(strtolower($tagLib)));
        if($tLib->valid()) {
            //如果标签库有效则取出支持标签列表
            ...
            foreach($tagList as $tag) {
                ...
                if($tag['nested'] && $this->config['tag_level']>1)
                    ...
                    $level   =   1;
                $endTag = $startTag;
                if(false !== stripos($content,S('TAGLIB_BEGIN').$startTag)) {
    ...
    public function parseTag($tagStr){
        ...
        if(preg_match('/^[\s|\d]/is',$tagStr))
            //过滤空格和数字打头的标签
            return S('TMPL_L_DELIM') . $tagStr .S('TMPL_R_DELIM');
        ...
        if(!empty($args)) {
            ...
            switch($tag){
                ...
                //…………
                default:
                    if(S('TAG_EXTEND_PARSE')) {
                        $method = S('TAG_EXTEND_PARSE');
    ...
    public function parseVar($varStr){
        ...
        if(!empty($varStr)){
            ...
            elseif( false !== strpos($var,'.')) {
                ...
                switch(strtolower(S('TMPL_VAR_IDENTIFY'))) {
                    case 'array': // 识别为数组
                        $name = '$'.$var;
    ...
    public function parseVarFunction($name,$varArray){
        ...
        //取得模板禁止使用函数列表
        $template_deny_funs = explode(',',S('TMPL_DENY_FUNC_LIST'));
        for($i=0;$i<$length ;$i++ ){
