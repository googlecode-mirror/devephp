...
class DbPdo extends Db{
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            $this->connected    =   true;
            // 注销数据库连接配置信息
            if(1 != S('DB_DEPLOY_TYPE')) unset($this->config);
        }
    ...
    public function getFields($tableName) {
        $this->initConnect(true);
        if(S('DB_DESCRIBE_TABLE_SQL')) {
            // 定义特殊的字段查询SQL
            $sql   = str_replace('%table%',$tableName,S('DB_DESCRIBE_TABLE_SQL'));
        }else{
            switch($this->dbType) {
                case 'MSSQL':
    ...
    public function getTables($dbName='') {
        if(S('DB_FETCH_TABLES_SQL')) {
            // 定义特殊的表查询SQL
            $sql   = str_replace('%db%',$dnName,S('DB_FETCH_TABLES_SQL'));
        }else{
            switch($this->dbType) {
            case 'ORACLE':
            case 'OCI':
