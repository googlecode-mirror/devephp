...
function __autoload($name)
{
    ...
    if(substr($name,0,6)=='Common'){
	    require_cache(LIB_PATH.$name.'.class.php'); 
	}elseif(substr($name,0,5)=="Admin"){
        require_cache(LIB_PATH.$name.'.class.php');
	}elseif(substr($name,-5)=="Model"){
        require_cache(MODULE_PATH.'Model/'.$name.'.class.php');
    }elseif(substr($name,-6)=="Action"){
        require_cache(MODULE_PATH.'Action/'.$name.'.class.php');
    }else {
        // 根据自动加载路径设置进行尝试搜索
...
// 远程调用模块的操作方法
function R($module,$control,$action,$app='C') {
