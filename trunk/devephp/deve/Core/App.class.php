...
{//类定义开始
	...
	static public function init()
	{
		...
		if(!defined('MODULE_NAME')) define('MODULE_NAME',   App::getModule());        // Module 名称
		if(!defined('CONTROL_NAME'))define('CONTROL_NAME',  App::getControl());       // Control名称
		if(!defined('ACTION_NAME')) define('ACTION_NAME',   App::getAction());        // Action 操作

		// 加载模块配置文件
//	    if(is_file(CONFIG_PATH.'Document/'.strtolower(MODULE_NAME).'_config.php'))
//	        C(include CONFIG_PATH.'Document/'.strtolower(MODULE_NAME).'_config.php');
	...
	{
		// 加载惯例配置文件
		C(include DEVE_PATH.'/Config/Config.php');
		// 加载项目配置文件
		...
		$common   = '';
		// 是否为开发模式
		$runtime = !defined('IS_DEBUG') || IS_DEBUG == false;
		// 加载项目公共文件
		...
		if(C('APP_DEBUG')) {
			// 加载系统默认的开发模式配置文件
			C(include DEVE_PATH.'/Config/Debug.php');
			// 允许项目增加开发模式配置定义
	...
	static private function getControl()
	{
		...
		$control   = !empty($_POST[$var]) ?
			$_POST[$var] :
			(!empty($_GET[$var])?$_GET[$var]:C('DEFAULT_CONTROL'));
		unset($_POST[$var],$_GET[$var]);
		return $control;
	}
	...
	static private function checkTemplate()
	{
		...
		{
			define('__URL__',PHP_FILE.'?m='.$module);
			C('TMPL_FILE_NAME',TEMPLATE_PATH.'/'.MODULE_NAME.CONTROL_NAME.C('TMPL_TEMPLATE_SUFFIX'));
